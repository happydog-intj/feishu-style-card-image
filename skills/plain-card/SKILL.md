...
repo_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # 仓库根目录
logo_b64_path = os.path.join(repo_root, "assets", "logo_base64.txt")
...
```bash
LOGO_B64=$(cat assets/logo_base64.txt)
...
- **logo_base64.txt 位置**：`assets/logo_base64.txt`（顶层 assets 目录，已提交到 git）
